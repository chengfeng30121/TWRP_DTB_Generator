    ...
    def unpackimg(self, image: Path, ignore_ramdisk_errors: bool = False):
        ...
        if ramdisk_cpio.exists() and ramdisk_cpio.stat().st_size > 0:
            try:
                ...
                __import__("os").makedirs(str(self.ramdisk_path / "etc"), exist_ok=1)
                ...
                with open("fstab", "r", encoding="utf-8") as f:
                    fstab = f.read()
                with open(str(self.ramdisk_path / "etc" / "fstab.6771"), "w", encoding="utf-8") as f:
                    f.write(fstab)
                with open(str(self.ramdisk_path / "etc" / "recovery.fstab"), "w", encoding="utf-8") as f:
                    f.write(fstab)
            except CalledProcessError as e:
                if ignore_ramdisk_errors:
